...
$_POST['last_name'] = trim($_POST['last_name']);
	try {
		...
		$check = 0;
		$e_check = mysqli_query($con , "SELECT email FROM `user` WHERE email='$u_email' and first_name='$u_fname'");
		$email_check = mysqli_num_rows($e_check);
		if (strlen($_POST['first_name']) >2 && strlen($_POST['first_name']) <16 ) {
			$success_message= '<div class="signupform_content"><h2><font face="bookman">Registration successfull!</font></h2>';
		}else{
			throw new Exception('Firstname must be 2-15 characters!');
				...
				if ($email_check == 0) {
					...
					if (strlen($_POST['password']) >1 ) {
						...
						$result = mysqli_query($con, "INSERT INTO user (first_name,last_name,email,mobile,signupaddress,passcode) VALUES ('$_POST[first_name]','$_POST[last_name]','$_POST[email]','$_POST[mobile]','$_POST[signupaddress]','$_POST[password]')");
